...
/**
 * Utilidad para probar el sistema de diagnóstico y reparación de errores
 */
export class SystemTest {
  ...
  /**
   * Prueba rápida de conectividad usando GraphQL directo
   */
  static async quickConnectivityTest(): Promise<boolean> {
    try {
      const connectionTest = await testGraphQLConnection();
      return connectionTest.success && connectionTest.serverReachable;
    } catch (error) {
